    ...
    try:
        ...
        with mp_pose.Pose(static_image_mode=True, min_detection_confidence=0.5) as pose_model, \
            ...
            for seg in segments:
                ...
                # Audio metrics
                start_sample = int(start * sr) # type: ignore
                end_sample = int(end * sr) # type: ignore
                ...
                if len(seg_audio) > 0:
                    ...
                    speaking_rate = len(seg_text.split()) / (end - start) if (end - start) > 0 else 0 # type: ignore
                else:
                    avg_pitch = 0.0
                    energy = 0.0
                    speaking_rate = 0.0

                # Video metrics: frame at midpoint
                mid_frame_idx = int(((start + end) / 2) * fps) # type: ignore
                ...
                if not ret:
                    ...
                    if pose_results.pose_landmarks:
                        ...
                        try:
                            ...
                            head_y = nose.y
                            clarity_score = np.clip(1.0 - abs(head_y - shoulder_mid_y), 0, 1)
                        except:
                            clarity_score = 0.0
                    else:
                        clarity_score = 0.0
