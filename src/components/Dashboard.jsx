...
function Dashboard({ user }) {
  ...
  const sidebarItems = [
    ...
    { name: "Resume Analysis", icon: FileText, active: false },
    { name: "Interview Prep", icon: Video, active: false },
    { name: "Progress & Reports", icon: TrendingUp, active: false },
  ]
  ...
  return (
    <div className="min-h-screen bg-[#101622] flex">
      {/* Sidebar */}
      <div className="w-64 bg-[#101622] border-r border-slate-800 flex flex-col">
        {/* Logo */}
            ...
            {mainFeatures.map((feature, index) => {
              ...
              return (
                <div key={index} className="bg-[#101622] rounded-xl p-6 border border-slate-700">
                  ...
                  <button className="w-full bg-[#0C40A5] hover:bg-blue-700 text-white font-medium px-4 py-2 rounded-lg transition-colors text-sm flex items-center justify-center gap-2 cursor-pointer">
                    {feature.buttonText}
                      ...
                      <div className="flex items-center gap-1">
                        {[...Array(5)].map((_, i) => (
                          <Star
                            key={i}
                            className={`w-3 h-3 ${
                              i < interview.rating ? "text-yellow-400 fill-current" : "text-slate-500"
                            }`}
                          />
                        ))}
                      </div>
